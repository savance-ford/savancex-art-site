...
import { US_STATES, type ShippingAddressInput } from "@/lib/shipping/address";
import type {
...
export function CheckoutForm() {
  ...
  async function requestShippingRates(form: HTMLFormElement) {
    if (isQuoting || isSubmitting || !form.reportValidity()) return;
    const requestBody: ShippingQuoteRequest = {
      items,
      address: readShippingAddress(form),
    };
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const checkoutRequest: CheckoutRequest = {
      items,
      shippingAddress: readShippingAddress(event.currentTarget),
      shippingMethodId,
    };
  ...
  return (
    <main className="checkout-shell" id="main">
      <form className="checkout-main" onSubmit={handleSubmit}>
        ...
        <div className="form-grid" onChange={invalidateQuote}>
          <div className="field field--full">
            <label htmlFor="checkout-name">Full name</label>
            <input id="checkout-name" name="name" autoComplete="shipping name" required />
          </div>
          <div className="field field--full">
            <label htmlFor="checkout-email">Email</label>
            <input id="checkout-email" name="email" type="email" autoComplete="shipping email" required />
          </div>
          <div className="field field--full">
            <label htmlFor="checkout-address-1">Street address</label>
            <input id="checkout-address-1" name="addressLine1" autoComplete="shipping address-line1" required />
          </div>
          <div className="field field--full">
            <label htmlFor="checkout-address-2">Apartment, suite, unit, etc. (optional)</label>
            <input id="checkout-address-2" name="addressLine2" autoComplete="shipping address-line2" />
          </div>
          <div className="field">
          ...
          <div className="field">
            ...
            <select id="checkout-state" name="stateCode" autoComplete="shipping address-level1" defaultValue="" required>
          ...
          <div className="field">
            ...
            <input id="checkout-zip" name="postalCode" autoComplete="shipping postal-code" pattern="[0-9]{5}(-[0-9]{4})?" required />
          </div>
          <div className="field">
            <label htmlFor="checkout-country">Country</label>
            <select id="checkout-country" name="countryCode" autoComplete="shipping country" defaultValue="US" required>
